...
import { getQurbanCampaigns } from "@/lib/data/qurbanRepository";

export default async function AdminQurbanExportPage() {
  const campaigns = await getQurbanCampaigns();
      ...
      <section className="grid gap-6 lg:grid-cols-[1fr_0.72fr]">
        <form className="rounded-lg border border-border-soft bg-white p-5 shadow-sm">
          <div className="grid gap-5 md:grid-cols-2">
            ...
            <label className="text-sm font-bold text-dark-navy">Kampanya<select className="focus-ring mt-2 w-full rounded-2xl border border-border-soft px-4 py-3"><option>Tümü</option>{campaigns.map((campaign) => <option key={campaign.id}>{campaign.title}</option>)}</select></label>
            <label className="text-sm font-bold text-dark-navy">Ülke/bölge<input className="focus-ring mt-2 w-full rounded-2xl border border-border-soft px-4 py-3" placeholder="Türkiye, Afrika Bölgesi..." /></label>
            <label className="text-sm font-bold text-dark-navy">Durum<select className="focus-ring mt-2 w-full rounded-2xl border border-border-soft px-4 py-3"><option>Tümü</option><option>Vekalet bekliyor</option><option>Ödeme bekliyor</option><option>Kesim planlandı</option><option>Dağıtım tamamlandı</option></select></label>
        ...
        <aside className="grid gap-4">
          <div className="rounded-lg border border-border-soft bg-white p-5 shadow-sm">
            ...
            <p className="mt-2 text-sm leading-6 text-ink-muted">Export dosyalarında bağışçı adı, e-posta ve telefon alanları minimum görünürlük ilkesiyle hazırlanmalıdır.</p>
          </div>
          <div className="rounded-lg border border-border-soft bg-white p-5 shadow-sm">
